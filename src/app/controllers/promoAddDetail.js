    ...
    function ($scope, $state, dataTableResources, $mdDialog, getWebService, createConditionService, Notification, promoParamsService) {
      ...
      $scope.openDialog = function (button) {
        ...
        $mdDialog.show({
          ...
          controller: function ($mdDialog, $scope, Notification, dataTableResources, params) {
            ...
            function success(items) {
              ...
              if (params.comparingField && !params.comparingField.value) {
                //a.1)recupera il primo record selezionato che abbia quel campo valorizzato facendolo
                // diventare il riferimento
                params.comparingField.value = items.filter
                (function (item) {
                  return !!item[params.comparingField.model]
                })[0][params.comparingField.model];
              ...
              items.filter(function (item) {
                if (item[params.comparingField.model] == params.comparingField.value)
               return true;
                //se la stringa contiene più valori suddivisi da ',' verifica che almeno uno di essi sia uguale al
               //valore di riferimento
                if (item[params.comparingField.model].split(/\s*,\s*/).length > 1)
                {
                  return item[params.comparingField.model].split(/\s*,\s*/).indexOf(params.comparingField.value) > -1;
                }
              }).length === items.length)
              ) {
                ...
                var filteredItems = items.map(function (item) {
                  ...
                  angular.forEach(button.columns, function (column) {
                    ...
                    if (item[column.refModel]) {
                      //se c'è un valore di confronto già settato per il campo corrente, viene passato quello
                      if (params.comparingField && column.refModel === params.comparingField.model)
                      {
                        value = params.comparingField.value;
                      }
                      //altrimenti viene passato il valore selezionato
                      else {value = item[column.refModel]}
                      filteredItem.push(
                        {
                          model: column.model,
                          value: value,
              ...
              //se non tutti i record selezionati hanno il campo di paragone coincidente con quello atteso
                ...
                error = (isAlreadyComparingFieldValued)
                  ...
                  + dataTableResources[button.reference].columns.filter(function (column) {
                    return column.model === params.comparingField.model
                  })[0].title +
                  " \"" + params.comparingField.value + "\""
          ...
          //alla chiusura della pop-up ritornando sulla pagina principale
        }).then(
          function (filteredItems) {
            ...
            angular.forEach(filteredItems, function (fItem) {
              angular.forEach(fItem, function (field) {
                ...
                var valueList = field.value.split(/\s*,\s*/);
                //se è una lista di valori da visualizzazione come picklist
                if (valueList.length > 1) {
                  $scope.columns = $scope.columns.map(function (column) {
                    if (column.model === field.model) {
                      return {
                        model: column.model,
                        title: column.title,
                        type: 'options',
                  ...
                  $scope.newItem[field.model] = valueList[0];
                }
                else {
                  $scope.columns = $scope.columns.map(function (column) {
                    if (column.model === field.model) {
                  ...
                  //se il valore non va messo in aggiunta a quanto già presente nel campo
                  if (!field.append) {
                  ...
                  //altrimenti se deve andare in append
                  {
      ...
      //resetta/inizializza il form
      function reset() {

        $scope.columns = angular.copy(dataTableResources["promo.detail"][$scope.tipoPromo].columns);
        $scope.buttons = angular.copy(dataTableResources["promo.detail"][$scope.tipoPromo].buttons);
        if ($scope.promoBatch == "Y") {
          $scope.columns = $scope.columns.filter(function (col) {
            return !col.batchDisabled;
          });
          if ($scope.buttons) {
            $scope.buttons = $scope.buttons.filter(function (col) {
              return !col.batchDisabled;
            });
          }
        }
